...
### 任务 7：差异分析 + 优化机会 + 验证路线（Issue 桥接）— 待实施

通过 GitCode/GitHub Issue 桥接外网 LLM 服务，每个热点函数一个 issue。

产出：

- `pipelines/pyframework_pipeline/bridge/` 模块（IssueClient + 模板生成 + 评论解析 + Manifest）
- Issue 模板：提示词 + 源码 + objdump -S 机器码（参考 spark/issues/1 实际模板）
- LLM 评论解析：结构化 Markdown（总览 → 逐行分析 → 根因汇总 → 优化策略）
- 支持 GitHub 和 GitCode 双平台 API
- 边缘场景：超长函数分块、单平台函数降级

验收：

- `bridge publish --dry-run` 生成符合格式的 issue body
- `bridge publish` 在 GitCode/GitHub 创建 issue
- `bridge fetch` 拉取评论并解析为 Dataset.diffView / patterns / rootCauses / opportunities

设计文档：详见 `docs/plans/sorted-hatching-boot.md`（plan 文件）
