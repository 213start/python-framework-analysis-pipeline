...
## Implementation Status (session 2026-06-29)

实际落地的内容(均经实测验证,`PYTHONPATH=pipelines python3 -m unittest discover` = 279 tests,仅 2 个预先存在的 Windows 路径分隔符失败):

### Phase 1(机械整合)— 完成 ✅
commits: `abdf22d` / `dad6983` / `0f18b8a` / `9e90f5b`
- `pyframework_pipeline/analyze/` 子包建立,16 个 kits 脚本 + `cpython_category_rules.json` 搬入(provenance: vendor@7270bcea)。
- 9 套 kits 测试迁入 `pipelines/tests/`(import 路径改写、`RULES_PATH` 修正)。
- acquisition / compare / orchestrator 远程路径全部改用 `python -m pyframework_pipeline.analyze.X` 模块调用,不再依赖 `vendor/`。
- `vendor/python-performance-kits` submodule 删除,`.gitmodules` 移除,`_init_submodules` 置为 no-op。

### Phase 2(领域建模)— 完成 ✅
commits: `829b7e0` / `48a9966`
- `contracts/` 四个数据族落地:`records.py`(RawSample/PerfRecord)、`tables.py`(AggregatedTables + CategoryRow/SymbolRow/SharedObjectRow)、`instruction.py`(InstructionDataset)、`timing.py`(TimingDataset)。每个契约含必选序列化(`to_file`/`from_file`/`write_dir`/`read_dir`/`write_json`/`read_json`)。列名严格匹配 analyze 的实际 CSV/JSON schema。
