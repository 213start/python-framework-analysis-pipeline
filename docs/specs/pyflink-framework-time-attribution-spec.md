...
在新的四层抽象下，本规范的主要归属为 `Framework`，次要涉及 `Dataset`。

主要归属：

- `Framework`
  - 定义 PyFlink 的 Python 侧分析边界
  - 定义纳入范围与排除范围
  - 定义一级/二级分类体系
  - 定义样本进入正式结论链的准入原则
...
- `Dataset`
  - 具体 profiling 样本、函数热点、组件/分类耗时结果需要遵循本规范
  - 但这些样本本身不由本规范承载

明确不归属：

- `Source`
  - 本规范不定义源码仓、revision、源码文件、源码锚点和机器码附件
- `Project`
  - 本规范不定义 case、函数、pattern 与源码资产之间的绑定表

## 3. 迁移映射

在 PyFlink 参考实现迁移到 `Framework / Dataset / Source / Project` 四层模型时，本规范应主要沉淀到 `Framework` 对象中。
...
- `Framework.taxonomy`
  - 来自本规范的一级/二级分类体系
- `Framework.metricGuardrails`
  - 来自本规范的样本归属原则、判定顺序与 Unknown 告警阈值
- `Dataset`
  - 仅保存遵循这些规则后得到的具体结果，例如 component/category/function 统计

换句话说，本规范回答的是“一个 PyFlink 项目应该如何理解和归类 Python 侧框架耗时”，而不是“某次实验具体测到了什么结果”。
