...
在新的四层抽象下，本规范主要横跨 `Dataset`、`Source` 与 `Project` 三层。

主要归属：

- `Dataset`
  - 保存热点函数、模式、根因、机会排序等分析结果
  - 保存组件/分类/函数层的差异证据摘要

- `Source`
  - 保存源码片段、机器码片段、源码文件、symbol 索引和 artifact 索引

- `Project`
  - 保存函数、源码锚点、机器码区块、pattern、root cause 之间的显式绑定关系

明确不归属：

- `Framework`
  - 本规范不定义框架边界、分类体系和样本归属判定规则

## 3. 迁移映射

迁移到四层模型后，本规范建议按以下方式拆分：
...
- `Source`
  - `AsmSnippet`
  - `SourceSnippet`
  - 对应的源码文件、源码区间、机器码附件和索引

- `Project`
  - `functionBindings`
    - 把 hotspot function 绑定到源码锚点和平台机器码区块
  - `patternBindings`
    - 把 pattern 绑定到多个函数、源码锚点和附件
  - `rootCauseBindings`
    - 把 root cause 绑定到多个 pattern

这份规范的本质不是定义“某个根因长什么样的 UI 卡片”，而是定义“分析结果层、源码资产层、绑定层之间如何形成可追溯的证据链”。
