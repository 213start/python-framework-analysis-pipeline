...
from .perf_collector_data import perf_collector
import os
...
class OutputGraph(OutputGraphCommon):
    ...
    def mark_bytecode_tracing_start(self) -> None:
        ...
        if self._perf_enabled:
            self._perf_collector_id = perf_collector.start_collection(
                "bytecode_tracing",
                metadata={
                    "code": self.code.co_name if hasattr(self, 'code') else "unknown",
                    "filename": self.code.co_filename if hasattr(self, 'code') else "unknown",
                }
            )
    ...
    def mark_bytecode_tracing_stop(self) -> None:
        # 先停止 perf 采集
        if self._perf_enabled and self._perf_collector_id is not None:
            perf_collector.stop_collection(self._perf_collector_id)
            self._perf_collector_id = None
