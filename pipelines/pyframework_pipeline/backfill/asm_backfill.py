...
def _load_symbol_source_map(
    ...
    """Load the CPython symbol -> source mapping.

    Priority:
    1. Dynamic maps generated by _extract_cpython_sources (in run dirs)
    2. Static fallback from backfill/data/symbol_source_map.json
    """
    global _symbol_source_map
    if _symbol_source_map is None:
        _symbol_source_map = {}
        # Load dynamic maps from run directories (step 5b generated).
        if run_dirs:
            for rd in run_dirs:
                for sub in ("arm64", "arm", "x86_64", "x86"):
                    dynamic = rd / "asm" / sub / "symbol_source_map.json"
                    if not dynamic.exists():
                        # Also check perf/data/ where orchestrator saves it.
        ...
        if _SYMBOL_MAP_PATH.exists():
            try:
                with open(_SYMBOL_MAP_PATH, encoding="utf-8") as f:
                    static = json.load(f)
                    # Dynamic takes precedence; static fills gaps.
                    for k, v in static.items():
                        _symbol_source_map.setdefault(k, v)
            except (json.JSONDecodeError, OSError):
                pass
