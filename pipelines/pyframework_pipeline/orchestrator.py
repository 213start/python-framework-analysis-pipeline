...
def _run_benchmark(
    ...
    *, force: bool = False,
    ...
    if _framework_id(env_config) == "datajuicer":
        ...
        return
    if _framework_id(env_config) == "udfbenchmarking":
        _run_udfbenchmarking_benchmark(
            project_path, run_dir, platform, executor, workload, env_config, force=force,
        )
        return
...
def _run_udfbenchmarking_benchmark(
    project_path: Path,
    run_dir: Path,
    platform: str,
    executor: Any,
    ...
    *,
    ...
    container = _udfbenchmarking_container(env_config)
    benchmark_name = _udfbenchmarking_benchmark_name(env_config)
    config_file = _udfbenchmarking_config_file(env_config)
    platform_run_dir = run_dir / platform
    platform_run_dir.mkdir(parents=True, exist_ok=True)
    timing_path = platform_run_dir / "timing" / "timing-normalized.json"
    ...
    if timing_path.exists() and timing_path.stat().st_size > 0:
        ...
        _write_udfbenchmarking_timing_artifacts(
            raw_output_dir,
            platform_run_dir,
            platform,
            benchmark_name,
    ...
    _run_udfbenchmarking_python_flamegraph(
        executor,
        container,
        platform,
        platform_run_dir,
...
def _run_udfbenchmarking_python_flamegraph(
    executor: Any,
    container: str,
    platform: str,
    platform_run_dir: Path,
    ...
    config = _udfbenchmarking_python_flamegraph_config(env_config)
    if not config["enabled"]:
        return
...
def _write_udfbenchmarking_timing_artifacts(
    raw_output_dir: Path,
    platform_run_dir: Path,
    platform: str,
    benchmark_name: str,
